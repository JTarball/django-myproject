...
from project.utils import logger, handlers
...
DATABASES = {
    'default': {
        ...
        'NAME':     os.environ.get('POSTGRES_DB'),             # Or path to database file if using sqlite3.
...
WSGI_APPLICATION = 'run.application'
...
#STATIC_URL = '/static/'
#STATIC_HOST = '//d2rihczxuk7w9w.cloudfront.net' if not DEBUG else ''
#STATIC_URL = STATIC_HOST + '/static/'


STATIC_HOST = os.environ.get('DJANGO_STATIC_HOST', '')  # only needs to set for production
STATIC_URL = STATIC_HOST + '/static/'
...
ALLOWED_HOSTS = ['ec2-52-29-137-44.eu-central-1.compute.amazonaws.com', 'www.tetherboxapp.com']
...
# ---------------------------------------------------------------------------- #
# Celery Settings
BROKER_URL = 'amqp://%s:5672//' % os.environ.get('DOCKER_MACHINE_IP')
CELERY_RESULT_BACKEND = 'redis://%s' % os.environ.get('DOCKER_MACHINE_IP')
...
# Search App
REDIS_HOST = '%s' % os.environ.get('DOCKER_MACHINE_IP')
...
LOGGING = {
    ...
    'loggers': {
        'django': {
            'handlers': ['console'],
            'propagate': True,
            'level': os.environ.get('CUSTOM_DJANGO_LOGGING_SETTING'),
        },
        'django.request': {
